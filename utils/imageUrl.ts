export const imageUrl = (public_id: string, format: string, mosaic: boolean, sample: boolean) => {
    if (sample) {
        return `https://res.cloudinary.com/${process.env.NEXT_PUBLIC_CLOUDINARY_CLOUD_NAME}/image/upload/c_scale,w_512/w_400,c_fit,l_text:corp.ttf_40_bold:Sample/${public_id}.${format}`
    }
    if (mosaic) {
