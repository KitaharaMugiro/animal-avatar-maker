...
export default (props: Props) => {
    ...
    return (
        <div className="m-3 w-full max-w-xs overflow-hidden rounded-lg bg-white shadow-lg">
            <Image unoptimized className="h-56 w-full object-cover" src={image} width={500} height={500} alt="avatar" />

            <div className="py-5 text-center">
                <span className="block text-xl font-bold text-gray-800" html-tabindex="0" role="link">
                    {props.date}
                </span>
                <p className="text-sm text-gray-700">{statusText()}</p>
                <p className="mt-2 text-xs text-gray-700">作成完了時にメールで通知します</p>
            </div>
